"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var expressInstance = require('express');
var router = expressInstance.Router();
var Feature = require('../model/Feature');
// Get all Features - http://localhost:7000/api/features
router.get('/', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var allFeatures, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Feature.find()];
            case 1:
                allFeatures = _a.sent();
                res.status(200).send(allFeatures);
                return [3 /*break*/, 3];
            case 2:
                err_1 = _a.sent();
                res.status(400).send(err_1);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
// Create a new feature - http://localhost:7000/api/features/feature
router.post('/feature', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var reqData, feature, savedFeature, err_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                reqData = req.body;
                feature = new Feature({
                    feature_id: reqData.feature_id,
                    feature_name: reqData.feature_name,
                    feature_type: reqData.feature_type,
                    feature_description: reqData.feature_description,
                    feature_created_timestamp: reqData.feature_created_timestamp,
                    feature_version: reqData.feature_version,
                    feature_owner: reqData.feature_owner,
                    feature_data: reqData.feature_data
                });
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, feature.save()];
            case 2:
                savedFeature = _a.sent();
                res.status(200).send(savedFeature);
                return [3 /*break*/, 4];
            case 3:
                err_2 = _a.sent();
                res.status(400).send(err_2);
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
//Update Existing Feature -	http://localhost:7000/api/features/{feature_id}
router.patch('/:featureId', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var reqData, updateFeature, err_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                reqData = req.body;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, Feature.updateOne({ _id: req.params.featureId }, { $set: {
                            feature_id: reqData.feature_id,
                            feature_name: reqData.feature_name,
                            feature_type: reqData.feature_type,
                            feature_description: reqData.feature_description,
                            feature_created_timestamp: reqData.feature_created_timestamp,
                            feature_version: reqData.feature_version,
                            feature_owner: reqData.feature_owner,
                            feature_data: reqData.feature_data
                        } })];
            case 2:
                updateFeature = _a.sent();
                res.status(200).send(updateFeature);
                return [3 /*break*/, 4];
            case 3:
                err_3 = _a.sent();
                res.status(400).send(err_3);
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
//Remove Feature - http://localhost:7000/api/features/{feature_id}
router.delete('/:featureId', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var deleteFeature, err_4;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Feature.findByIdAndRemove({ _id: req.params.featureId })];
            case 1:
                deleteFeature = _a.sent();
                res.status(200).send('Feature Deleted : ' + deleteFeature);
                return [3 /*break*/, 3];
            case 2:
                err_4 = _a.sent();
                res.status(400).send(err_4);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
//Get Feature By id - http://localhost:7000/api/features/search/{feature_id}
router.get('/:featureId', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var currentFeature, err_5;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Feature.findById(req.params.featureId)];
            case 1:
                currentFeature = _a.sent();
                res.status(200).send(currentFeature);
                return [3 /*break*/, 3];
            case 2:
                err_5 = _a.sent();
                res.status(400).send(err_5);
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
module.exports = router;
